 ...
 * ```
 *
 * where `as tree` stands for attaching a behavior named `tree`, and the array will be passed to [[\Eii::createObject()]]
...
class Component extends BaseObject
{
    ...
    public function __set($name, $value)
    {
        ...
        if (method_exists($this, $setter)) {
            ...
            return;
        } elseif (0 === strncmp($name, 'on ', 3)) {
            // on event: attach event handler
            $this->on(trim(substr($name, 3)), $value);

            return;
        } elseif (0 === strncmp($name, 'as ', 3)) {
            // as behavior: attach behavior
            $name = trim(substr($name, 3));
            $this->attachBehavior($name, $value instanceof Behavior ? $value : Eii::createObject($value));
     ...
     *     Eii::trace($event->name . ' is triggered.');
     * });
     * ```
     *
     * @param string   $name    the event name
     ...
     *  - an object configuration array that will be passed to [[Eii::createObject()]] to create the behavior object.
     *
    ...
    private function attachBehaviorInternal($name, $behavior)
    {
        if (!($behavior instanceof Behavior)) {
            $behavior = Eii::createObject($behavior);
        }
        if (is_int($name)) {
