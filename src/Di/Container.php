 ...
 *
 * By default, [[\Eii::$container]] refers to a Container instance which is used by [[\Eii::createObject()]]
...
class Container extends Component
{
        ...
        if (null !== $constructor) {
            foreach ($constructor->getParameters() as $param) {
                if (version_compare(PHP_VERSION, '5.6.0', '>=') && $param->isVariadic()) {
                    ...
                    $dependencies[] = Instance::of(null === $c ? null : $c->getName());
                }
            }
        }
     ...
     *    // ...
     * }
     * Eii::$container->invoke($formatString, ['string' => 'Hello World!']);
     * ```
     *
    ...
    public function resolveCallableDependencies(callable $callback, $params = [])
    {
        ...
        foreach ($reflection->getParameters() as $param) {
            ...
            if (($class = $param->getClass()) !== null) {
                ...
                } elseif (isset(Eii::$app) && Eii::$app->has($name) && ($obj = Eii::$app->get($name)) instanceof $className) {
                    $args[] = $obj;
                } else {
    ...
    public function setDefinitions(array $definitions)
    {
        foreach ($definitions as $class => $definition) {
            if (is_array($definition) && 2 === count($definition) && array_values($definition) === $definition && is_array($definition[1])) {
    ...
    public function setSingletons(array $singletons)
    {
        foreach ($singletons as $class => $definition) {
            if (is_array($definition) && 2 === count($definition) && array_values($definition) === $definition) {
